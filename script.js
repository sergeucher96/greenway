document.addEventListener('DOMContentLoaded', () => {
    ...
    const tests = [
        {
            ...
            timeLimit: 60,
            levelDescriptions: {
                'Elementary (A1)': "Понимать известные слова и очень простые предложения, если говорят отчетливо и медленно. Понимать отдельные известные слова и очень простые предложения, напр. на вывесках, плакатах и в каталогах. Говорить простыми предложениями, на простом уровне выражать свои мысли. Написать короткую почтовую открытку или поздравление и т. д.",
                'Pre-Intermediate (A2)': "Понимать очень простую информацию. Понимать самое главное в кратких, ясных и простых сообщениях. В простых текстах (проспектах, меню, расписании движения транспорта) находить нужную информацию и понимать письма личной переписки. Выражать свои мысли в простых ситуациях. При помощи нескольких предложений описать вашу личную и профессиональную жизнь. Писать простые короткие заметки или сообщения. Написать простое письмо личной переписки.",
                'Intermediate (B1)': "Понимать главную информацию, если речь идет о работе, школе, свободном времяпровождении. Понимать передачи, если говорят отчетливо. Понимать тексты бытовой и профессиональной сферы. Понимать письма, в которых речь идет о событиях и желаниях. Участвовать в разговорах о семье, хобби, работе, путешествиях или актуальных событиях. Писать простые тексты об известных событиях. В личной переписке сообщать о своих впечатлениях, переживаниях.",
                'Upper Intermediate (B2)': "Следить за ходом мыслей в длинных речах и докладах, понимать программы новостей, репортажи и художественные фильмы. Читать статьи и сообщения и понимать позицию автора. Понимать современные литературные произведения. Выражать свои мысли и участвовать в разговорах и дискуссиях с носителями языка. В сообщениях передавать информацию и формулировать аргументы."
            },
    ...
    // DOM Elements
    const screens = { home: document.getElementById('home-screen'), welcome: document.getElementById('welcome-screen'), test: document.getElementById('test-screen'), result: document.getElementById('result-screen'), review: document.getElementById('review-screen') };
    ...
    const homeFromResultsButton = document.getElementById('home-from-results-button');
    const restartCurrentTestButton = document.getElementById('restart-current-test-button');
    ...
    function initHome() {
        ...
        tests.forEach(test => {
            ...
            testElement.innerHTML = `<h3>${test.title}</h3><p>${test.description}</p>`;
            testElement.onclick = () => showWelcomeScreen(test.id);
    ...
    function shareResults() {
        if (BOT_USERNAME === 'YOUR_BOT_USERNAME_HERE' || !BOT_USERNAME) {
            tg.showAlert('Пожалуйста, укажите юзернейм вашего бота в файле script.js');
            return;
        }
    ...
    function showReview() {
        ...
        currentTest.questions.forEach((q, index) => {
            ...
            if(isCorrect){
                html += `<div class="user-answer correct-answer">Ваш ответ: ${userAnswer} ✔️</div>`;
            } else {
                html += `<div class="user-answer incorrect">Ваш ответ: ${userAnswer} ❌</div><div class="correct-answer">Правильный ответ: ${q.answer}</div>`;
            }
            html += `</div>`;
            item.innerHTML = html;
    ...
    // Event Listeners
    coursesBtn.onclick = () => { tg.showAlert("Раздел 'Курсы' находится в разработке. Следите за обновлениями!"); };
    lessonsBtn.onclick = () => { tg.showAlert("Раздел 'Уроки' находится в разработке. Скоро здесь появятся интерактивные материалы!"); };
    teachersBtn.onclick = () => { tg.showAlert("Информация для преподавателей скоро будет доступна в этом разделе."); };
    bookLessonBtn.onclick = () => { tg.showConfirm("Вы хотите перейти в чат для записи на урок?", (confirmed) => { if (confirmed) { tg.openTelegramLink(YOUR_CONTACT_LINK); } }); };
    startTestButton.onclick = startTest;
    backToMenuButton.onclick = initHome;
    homeFromResultsButton.onclick = initHome;
    restartCurrentTestButton.onclick = startTest;
    backButton.onclick = goBack;
    shareResultsButton.onclick = shareResults;
    reviewAnswersButton.onclick = showReview;
    backToResultsButton.onclick = () => showScreen('result');
